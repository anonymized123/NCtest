...
def main(dataset_name, model_name, seed):
    ...
    path_mutation_edge_index_np_list = '../data/{}/mutation_edge_index_np_list.pkl'.format(dataset_name)
    path_mutation_x_np_list = '../data/{}/mutation_x_np_list.pkl'.format(dataset_name)
    
    target_hidden_channel = 16
    
    # path_result_pfd = 'result/pfd' + '_' + subject_name + '.csv'
    path_result_apfd = 'result/NodeRank_{}.csv'.format(seed)
    ...
    feature_node_edge_np, label_node_edge_np = get_mutation_model_edge_features(num_node_features, target_hidden_channel, num_classes, target_model_path, x, y, edge_index, mutation_edge_index_list, model_name)

    # mutant node attribute feature
    mutation_attribute_index_list = pickle.load(open(path_mutation_x_np_list, 'rb'))
    feature_node_attribute_np, label_node_attribute_np = get_mutation_model_attribute_features(num_node_features, target_hidden_channel, num_classes, target_model_path, x, y, edge_index, mutation_attribute_index_list, model_name)
    ...
    idx_miss_list_train = get_idx_miss_class(target_pre_train, train_idx)
    ...
    # res_list = [fusion_apfd]
    # df = pd.DataFrame(columns=None, data=res_list)
    # df.to_csv(path_result_apfd, mode='a', header=False, index=False)


    # Different model fusion methods
    # y_pred_test_fusion_weight = y_pred_test_xgb*apfd(idx_miss_list_train, xgb_rank_idx_train) + y_pred_test_lr*apfd(idx_miss_list_train, lr_rank_idx_train) + y_pred_test_rf*apfd(idx_miss_list_train, rf_rank_idx_train) + y_pred_test_lgb*apfd(idx_miss_list_train, lgb_rank_idx_train)
    ...
    stacking_ratio_list = get_res_ratio_list(idx_miss_list, stacking_rank_idx, select_ratio_list)
    voting_ratio_list = get_res_ratio_list(idx_miss_list, voting_rank_idx, select_ratio_list)
    
    rank_idx_dict = {
        'stacking': stacking_rank_idx,
        'voting': voting_rank_idx
    }
    with open('result/NodeRank_rank_idx_{}_{}_{}.pkl'.format(dataset_name, model_name, seed), 'wb') as f:
        pickle.dump(rank_idx_dict, f)
        
    return  [fusion_stacking_apfd, fusion_voting_apfd], [stacking_ratio_list, voting_ratio_list]
...
if __name__ == '__main__':
    ...
    columns = ['stacking_apfd', 'voting_apfd']
    ...
    if model_name == 'gat' and dataset_name == 'citeseer':
        df_apfd.to_csv('result/NodeRank_{}.csv'.format(seed), mode='w', header=True, index=False)
        
    else:
        df_apfd.to_csv('result/NodeRank_{}.csv'.format(seed), mode='a', header=False, index=False)
    
        
    if model_name == 'gat' and dataset_name == 'citeseer':
        df_pfdn.to_csv('result/NodeRank_pfdn_{}.csv'.format(seed), mode='w', header=True, index=False)
        
    else:
        df_pfdn.to_csv('result/NodeRank_pfdn_{}.csv'.format(seed), mode='a', header=False, index=False)